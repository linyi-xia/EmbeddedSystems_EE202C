...
def route(database, destinationID):
	dict = {}
	...
	for i in range(1, len(destination.path)):
		instr = getInstruction(currentstate, getDirection(destination.path[i - 1], destination.path[i]))
		instructions += instr
	...
	cx.execute("UPDATE car_locations set Turn_by_Turn = ?, Dir = ?, Status = ? where Car_ID = ?", (instructions, currentstate, 1, 1001))
	cx.commit()
...
def getInstruction(state1, state2):
	instrct = (state2 - state1 + 4) % 4
	if instrct == 0: return "F"
	if instrct == 1: return "R"
	if instrct == 2: return "B"
	if instrct == 3: return "L" 
	

route("locationsNroutes.db", 2003)
